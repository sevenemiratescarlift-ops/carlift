...
export function FaqForm({ faq, action }: { faq?: Faq; action: (prevState: State, formData: FormData) => Promise<State> }) {
  ...
  return (
    <form action={formAction} className="space-y-5">
      ...
      <div>
        ...
        <textarea name="answer" required rows={4} defaultValue={faq?.answer} className={inputClass} />
      </div>
      <div className="flex flex-wrap items-center gap-6">
        <div>
          <label className={labelClass}>Display Order</label>
          <input type="number" name="displayOrder" defaultValue={faq?.displayOrder ?? 0} className={inputClass} />
        </div>
        <label className="flex items-center gap-2 text-sm text-slate-200">
          <input type="checkbox" name="published" defaultChecked={faq?.published ?? true} className="h-4 w-4 accent-[#00B4D8]" />
          Published
        </label>
      </div>
