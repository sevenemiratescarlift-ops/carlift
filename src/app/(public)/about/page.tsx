...
const VALUES = [
  { title: "Safety First", description: "Every trip is handled by licensed, background-checked professional drivers.", icon: "🛡️" },
  { title: "Punctuality", description: "We track flights and traffic to make sure you're never left waiting.", icon: "⏱️" },
  { title: "Comfort", description: "A modern, well-maintained fleet designed for a smooth, relaxing ride.", icon: "🛋️" },
  { title: "Transparency", description: "Clear communication on WhatsApp before every booking is confirmed.", icon: "💬" },
];

export default async function AboutPage() {
  const [homepage, site] = await Promise.all([getHomepageSettings(), getSiteSettings()]);

  return (
    <div className="mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8">
      <div className="grid gap-12 lg:grid-cols-2 lg:items-center">
        <Reveal direction="right">
          <p className="text-xs font-bold uppercase tracking-[0.2em] text-[var(--color-accent)]">About {site.companyName}</p>
        ...
        <Reveal direction="left" delay={0.1}>
          <div className="relative aspect-[4/3] overflow-hidden rounded-3xl border border-[var(--color-border)]">
            <Image src="/images/about-dubai.jpg" alt="Premium car service in Dubai" fill sizes="(max-width: 1024px) 100vw, 50vw" className="object-cover" />
          </div>
        </Reveal>
      </div>
